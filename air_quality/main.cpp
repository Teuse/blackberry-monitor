...
float sampleRateMode = BSEC_SAMPLE_RATE_LP;
...
int64_t get_timestamp_us()
{
	...
	int64_t system_current_time_ns = (int64_t)(spec.tv_sec) * (int64_t)1000000000 + (int64_t)(spec.tv_nsec);
						...
						float static_iaq, float stabStatus, float runInStatus, float co2_equivalent, float breath_voc_equivalent)
{
	...
	influxdb_cpp::builder()
		 ...
		 .field("iaq", iaq)
		 .field("iaq_accuracy", iaq_accuracy)
		 .field("temperature", temperature)
		 .field("humidity", humidity)
		 .field("pressure", pressure)
		 .field("gas", gas)
		 .field("bsec_status", bsec_status)
		 .timestamp(timestamp)
		 .post_http(si);
}
